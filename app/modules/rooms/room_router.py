...
async def view_room(
    ...
    room_dict = RoomResponse.model_validate(room_model).model_dump(mode="json")
...
async def view_all_rooms(
    ...
    # ---- 3. SAVING TO CACHE FOR NEXT TIME ---- #
    # Turn the Pydantic model into a dictionary so json.dumps can read it safely
    room_list_dict = [RoomResponse.model_validate(room).model_dump(mode="json") for room in room_model]
